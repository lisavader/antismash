...
def format_domain_description(cds_name, prediction) -> Markup:
    description = hmm_properties[prediction.type].description
    return Markup(f'<span class="serif">{cds_name}</span> ({prediction.start}-{prediction.end}): {description}')
...
def format_subtype(prediction) -> str:
    ...
    for subtype in prediction.subtypes:
        try:
            short_description = "".join(hmm_properties[subtype].description.split("; ")[1:])
        except:
...
def format_reactions(prediction) -> Markup:
    ...
    for reaction in prediction.reactions:
        ...
        for product in products:
            markup += Markup(f"{', '.join(substrates)} &rarr; {product}<br>")
    return markup
...
def generate_html(region_layer: RegionLayer, results: TerpeneResults,
    ...
    for cluster in region_layer.get_unique_protoclusters():
        if cluster.product_category == "terpene":
            protoclusters.append(results.cluster_predictions[cluster.get_protocluster_number()])
    ...
    details = details_template.render(protoclusters = protoclusters, tooltip = tooltip,
                                      format_domain_description = format_domain_description,
